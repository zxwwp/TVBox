var rule = {
    ...
    double:false,
    推荐:'*',
    一级:"js:var items=[];pdfh=jsp.pdfh;pdfa=jsp.pdfa;pd=jsp.pd;var html=request(input);var tabs=pdfa(html,'body&&tr.against');tabs.forEach(function(it){var pz=pdfh(it,'.against td:gt(3):lt(1)&&Text');var ps=pdfh(it,'a&&Text');var pk=pdfh(it,'.against td:gt(5):lt(6)&&Text');var img=pd(it,'img&&src');var timer=pdfh(it,'.against td:gt(1):lt(1)&&Text');var url=pd(it,'a&&href');items.push({desc:timer+'  '+ps,title:pz+pk,pic_url:img,url:url})});setResult(items);",
    二级:{
