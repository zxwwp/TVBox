var rule = {
    ...
    double:false,
    推荐:'*',
    一级:"js:var items=[];pdfh=jsp.pdfh;pdfa=jsp.pdfa;pd=jsp.pd;var html=request(input);var tabs=pdfa(html,'body&&tr.against');tabs.forEach(function(it){var pz=pdfh(it,'tr.against td:eq(5)&&Text');var ps=pdfh(it,'td.matcha&&Text');var pk=pdfh(it,'.category&&Text');var img=pd(it,'img&&src');var timer=pdfh(it,'time&&Text');var url=pd(it,'.links a&&href');items.push({desc:timer+'  '+ps+'🆚'+pk,title:pz,pic_url:img,url:url})});setResult(items);",
    二级:{
