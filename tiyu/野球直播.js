var rule = {
    ...
    一级:"js:var items=[];pdfh=jsp.pdfh;pdfa=jsp.pdfa;pd=jsp.pd;var html=request(input);var tabs=pdfa(html,'body&&ul.contest_list.li');tabs.forEach(function(it){var  pz=pdfh(it,'.bs_duiwu:ep(0)&&Text');var ps=pdfh(it,'span&&Text');var pk=pdfh(it,'.bs_duiwu:eq(1)&&Text');var img=pd(it,'img&&data-original');var timer=pdfh(it,'.time&&Text');var url=pd(it,'li.play a&&href');items.push({desc:timer+'  '+ps,title:pz+'🆚'+pk,pic_url:img,url:url})});setResult(items);",
