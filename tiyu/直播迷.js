var rule = {
    ...
    double:false,
    推荐:'*',
    一级:"js:var items=[];pdfh=jsp.pdfh;pdfa=jsp.pdfa;pd=jsp.pd;var html=request(input);var tabs=pdfa(html,'body&&.panel.panel-default.goto');tabs.forEach(function(it){var pz=pdfh(it,'ul.sc li:eq(4)&&Text');var ps=pdfh(it,'a&&Text');var pk=pdfh(it,'tr.against td:eq(7) strong&&Text');var img=pd(it,'img&&src');var timer=pdfh(it,'title&&Text');var url=pd(it,'a&&href');items.push({desc:timer+'  '+ps,title:pz+'🆚'+pk,pic_url:img,url:url})});setResult(items);",
    二级:{
