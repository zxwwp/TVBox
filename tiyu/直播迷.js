var rule = {
    ...
    double:false,
    推荐:'*',
    一级:"js:var items=[];pdfh=jsp.pdfh;pdfa=jsp.pdfa;pd=jsp.pd;var html=request(input);var tabs=pdfa(html,'body&&ul.sc li:gt(0):lt(6)');tabs.forEach(function(it){var pz=pdfh(it,'ul.sc li:gt(4)&&Text');var ps=pdfh(it,'ul.sc li:gt(0)&&Text');var pk=pdfh(it,'ul.sc li:gt(6)&&Text');var img=pd(it,'img&&src');var timer=pdfh(it,'ul.sc li:gt(1)&&Text');var url=pd(it,'a&&href');items.push({desc:timer+'  '+ps,title:pz+'🆚'+pk,pic_url:img,url:url})});setResult(items);",
    二级:{
