var rule = {
    ...
    一级:"js:var items=[];pdfh=jsp.pdfh;pdfa=jsp.pdfa;pd=jsp.pd;var html=request(input);var tabs=pdfa(html,'body&&.post');tabs.forEach(function(it){var  pz=pdfh(it,'h2&&Text');var pz=pdfh(it,'.&&Text');var pz=pdfh(it,'.&&Text');var img=pd(it,'.bs_duiwu img&&data-original');var timer=pdfh(it,'.&&Text');var url=pd(it,'a&&href');items.push({desc:title:pz,pic_url:img,url:url})});setResult(items);",
