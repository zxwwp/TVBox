var rule = {
    ...
    二级:{
        ...
        content:'.biaoti&&Text',
        tabs:"js:TABS=['【直播源】']",
        lists:"js:LISTS=[];pdfh=jsp.pdfh;pdfa=jsp.pdfa;pd=jsp.pd;let html=request(input);let data=pdfa(html,'body#green&&dd');TABS.forEach(function(tab){let d=data.map(function(it){let name=pdfh(it,'span.diss&&Text');let url=pd(it,'dd&&nz-g-ca');return name+'$'+url});LISTS.push(d)});"
        },
    搜索:'',
}