var rule = {
    ...
    class_url:'/&live_1.html?project=1&yczb.html',    //静态分类标识拼接
    headers:{
    ...
    推荐:'*',
      //一级:'ul.match-list li;li div:ge(1):lt(3)&&Text;img&&src;div&&col-time;a&&href',
    //二级:'*', 
// 类似海阔一级 列表;标题;图片;描述;链接;详情 其中最后一个参数选填
    一级:"js:var items=[];pdfh=jsp.pdfh;pdfa=jsp.pdfa;pd=jsp.pd;var html=request(input);var tabs=pdfa(html,'body&&li.match-item');tabs.forEach(function(it){var  pz=pdfh(it,'.col-home.flex&&Text');var ps=pdfh(it,'.col-name&&Text');var img=pd(it,'img&&src');var timer=pdfh(it,'.col-time&&Text');var url=pd(it,'a&&href');items.push({desc:timer+'  '+ps,title:pz,pic_url:img,url:url})});setResult(items);",
 // 二级可以是*,表示规则无二级,直接拿一级的链接进行嗅探


    // 或者 {title:'',img:'',desc:'',content:'',tabs:'',lists:'',tab_text:'body&&Text',list_text:'body&&Text',list_url:'a&&href'} 同海阔dr二级
    二级:{
        "title":".play-info play-info-soccer&&Text",  // 二级 title: 片名;类型
        "desc":";;;div.col-home:eq(0)&&Text;div.col-home:eq(1)&&Text", // 二级 desc: 主要信息;年代;地区;演员;导演
        "content":"div.col-time&&Text", // 二级 content: 简介
        "tabs":"js:TABS=['直播信号源']",
