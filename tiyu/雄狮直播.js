var rule = {
    ...
    double:false,
    推荐:'*',
      一级:'.conbox.fn-left li;.tit&&p:lt(2)&&Text;img&&op-zfr-a-g;a&&sbtsw;a&&href', 
      二级:'*',
   ...
   // 二级:{title:"script&&Text",desc:";;;.nameOne&&Text;.nameTwo&&Text",content:"div.downinfo-detail:ep(2)&&Text",tabs:"js:TABS=['【直播源】']",lists:"js:LISTS=[];pdfh=jsp.pdfh;pdfa=jsp.pdfa;pd=jsp.pd;let html=request(input);let data=pdfa(html,'.downinfo-detail&&a');TABS.forEach(function(tab){let d=data.map(function(it){let name=pdfh(it,'rel&&Text');let url=pd(it,'a&&href');return name+'$'+url});LISTS.push(d)});"},
   搜索:'',
}