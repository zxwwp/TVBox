var rule = {
    ...
    一级:"js:var items=[];pdfh=jsp.pdfh;pdfa=jsp.pdfa;pd=jsp.pd;var html=request(input);var tabs=pdfa(html,'body&&.contenTab');tabs.forEach(function(it){var pz=pdfh(it,'.team:eq(1)&&p');var ps=pdfh(it,'.matchType&&Text');var pk=pdfh(it,'.team:eq(2)&&p');var img=pd(it,'img&&src');var timer=pdfh(it,'.date today&&Text');var url=pd(it,'a&&href');items.push({desc:timer+'  '+ps,title:pz+'🆚'+pk,pic_url:img,url:url})});setResult(items);",
