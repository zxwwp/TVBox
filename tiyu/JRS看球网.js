var rule = {
    ...
    一级:"js:var items=[];pdfh=jsp.pdfh;pdfa=jsp.pdfa;pd=jsp.pd;var html=request(input);var tabs=pdfa(html,'body&&.listBox');tabs.forEach(function(it){var pz=pdfh(it,'.myList div:gt(1)&&Text');var ps=pdfh(it,'.matchType&&Text');var img=pd(it,'img&&src');var timer=pdfh(it,'.hot s&&Text');var url=pd(it,'a&&href');items.push({desc:timer+'  '+ps,title:pz,pic_url:img,url:url})});setResult(items);",
