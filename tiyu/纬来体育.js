var rule = {
    ...
    推荐:'*',
      ...
      一级:"js:var items=[];pdfh=jsp.pdfh;pdfa=jsp.pdfa;pd=jsp.pd;var html=request(input);var tabs=pdfa(html,'body&&div');tabs.forEach(function(it){var  pz=pdfh(it,'p.once-game&&Text');var ps=pdfh(it,'p.once-event&&Text');var img=pd(it,'img&&data-original');var timer=pdfh(it,'p.once-time&&Text');var url=pd(it,'a&&href');items.push({desc:timer+'  '+ps,title:pz,pic_url:img,url:url})});setResult(items);",
