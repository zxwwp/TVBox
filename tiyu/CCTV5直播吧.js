var rule = {
    ...
    一级:"js:var items=[];pdfh=jsp.pdfh;pdfa=jsp.pdfa;pd=jsp.pd;var html=request(input);var tabs=pdfa(html,'body&&dd.clearfix div');tabs.forEach(function(it){var  pz=pdfh(it,'div.once-type:eq(2)&&Text');var ps=pdfh(it,'div.once-type:eq(1)&&Text');var img=pd(it,'img&&src');var timer=pdfh(it,'div.once-type:eq(0)&&Text');var url=pd(it,'div.video-link a&&href');items.push({desc:timer+'  '+ps,title:pz,pic_url:img,url:url})});setResult(items);",
