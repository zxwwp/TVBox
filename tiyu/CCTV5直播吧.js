var rule = {
    ...
    一级:"js:var items=[];pdfh=jsp.pdfh;pdfa=jsp.pdfa;pd=jsp.pd;var html=request(input);var tabs=pdfa(html,'body&&dd.clearfix div');tabs.forEach(function(it){var  pz=pdfh(it,'.once-game &&Text');var ps=pdfh(it,'.once-event&&Text');var img=pd(it,'img&&src');var timer=pdfh(it,'.once-time&&Text');var url=pd(it,'li.play a&&href');items.push({desc:timer+'  '+ps,title:pz,pic_url:img,url:url})});setResult(items);",
